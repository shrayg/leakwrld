...
The Node static handler sends **`Cache-Control: public, max-age=10800`** (3 hours) for files under **`/thumbnails/`** (creator grid **WebP** files in `client/public/thumbnails/*.webp`, generated by **`npm run thumbs:gen`**) so updates propagate within a few hours. Hashed **`/assets/`** and **`/fonts/`** still use **`max-age=31536000, immutable`**.

Regenerate thumbs after R2 changes: **`npm run thumbs:gen`** (missing only) or **`npm run thumbs:gen:force`**. Requires **ffmpeg** with **libwebp**. Legacy **`*.jpg`** files in that folder are removed when a new **`.webp`** is written.
...
**Short-card grid previews** still load **full R2 objects** via `mediaUrl(key)` until separate preview derivatives exist.
